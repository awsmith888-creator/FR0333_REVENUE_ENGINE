...
DELTA_PATH = HERE / "fr0333_delta_1_0_chat_to_work_bridge_upgrade.json"

REPOSITORY_TERMS = (
    "bridge", "repo", "repository", "github", "commit", "branch",
    "pull request", "write it in", "write the", "upload it", "into the system",
)
IMAGE_TERMS = ("image", "images", "picture", "pictures", "photo", "photos", "render")
IMAGE_EDIT_TERMS = ("edit", "fix", "clean up", "cleanup", "remove", "replace", "change", "remake")
IMAGE_GENERATION_TERMS = ("create", "generate", "make", "draw", "render", "visualize")
RESEARCH_TERMS = ("look up", "research", "statistics", "history", "search", "verify", "source")
STOP_PATTERNS = (r"\bstop\b", r"\bcancel\b", r"\bdon't\b", r"\bdo not\b", r"\bquit\b")
...
def detect_lane(text: str, prior_lane: Optional[str] = None) -> IntentReceipt:
    ...
    cancelled_lane = prior_lane if explicit_stop and prior_lane in IMAGE_LANES else None
    ...
    return IntentReceipt(
        ...
        state="T.20",
        execution_claim="REPOSITORY_ROUTE_CLASSIFICATION_ONLY",
    )


def tool_gate(receipt: IntentReceipt, requested_tool_class: str) -> str:
    return "T.20" if requested_tool_class in LANE_TOOLS[receipt.compiled_lane] else "F.6"
...
def load_control_records() -> tuple[dict, dict]:
    spec = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
    delta = json.loads(DELTA_PATH.read_text(encoding="utf-8"))
    assert spec["identifier"] == "FR0333.CHAT.TO.WORK.BRIDGE.0001"
    assert spec["delta_address"] == "∆.1.0"
    assert spec["reference_point"]["identifier"] == "RP.1"
    assert spec["reference_point"]["sha"] == "5c02d0c313f53244fd98dccffe11bbb97f6be2e1"
    assert spec["humanlock"] is True
    assert spec["humanlock_authorization"] == "GRANTED_FOR_THIS_REPOSITORY_UPGRADE"
    ...
    assert "NO_CROSS_LANE_FALLTHROUGH" in spec["override_rules"]
    assert delta["delta_address"] == "∆.1.0"
    assert delta["reference_point"]["sha"] == spec["reference_point"]["sha"]
    return spec, delta


def main() -> int:
    parser = argparse.ArgumentParser(description="FR0333 Chat-to-Work lane bridge ∆.1.0")
    ...
    payload = {"delta_address": "∆.1.0", "reference_point": "RP.1", "receipt": asdict(receipt)}
    if args.tool_class:
        payload["tool_gate"] = {"requested_tool_class": args.tool_class, "state": tool_gate(receipt, args.tool_class)}
